...
merton_params <- data.frame(
  Amount = c("Interest rate", "Firm asset value", "Time to maturity", "Default Threshold",
...
# Process table with descending PD, then unique dates
table_data <- data_1 %>%
  select(Date, Close, EstimatedAssetVolatility, ApproxDefaultProbability) %>%
  rename(
    `Stock Price (Close)` = Close,
    `Asset Volatility (σA)` = EstimatedAssetVolatility,
    `Approx. Default Probability` = ApproxDefaultProbability
  ) %>%
  arrange(desc(`Approx. Default Probability`)) %>%
  distinct(Date, .keep_all = TRUE) %>%  # keep the highest PD per date
  mutate(across(where(is.numeric), ~ formatC(.x, format = "f", digits = 10, big.mark = ",")))

# Show top 20 rows
head(table_data, 20) %>%
  kable("html", booktabs = TRUE, caption = "KMV Model Output") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE)
